...
本システムを使用・導入する前に、以下の点にご留意ください。

* **AI生成と手作業のハイブリッド**: 本コードベースはAIによって生成されたものをベースにしていますが、**少なからず手作業による修正・機能追加**が行われています。AI単独の出力ではバグが多く、実用的な機能の実装が困難であったため、人間による介入が入っています。
* **監修者について**: 本システムの監修は**公務員獣医師**が行っています。民間の動物病院での臨床経験がないため、実際の民間診療のワークフローや商習慣とは一部ズレが生じている可能性があります。実運用の際は、現場のニーズに合わせて適宜カスタマイズを行ってください。
