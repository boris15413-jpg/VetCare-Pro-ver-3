...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        ...
        if ($id) {
            unset($data['status'], $data['ordered_by']);
            $db->update('orders', $data, 'id=?', [$id]);
            setFlash('success', 'オーダーを更新しました');
        } else {
            $data['ordered_at'] = date('Y-m-d H:i:s');
            $db->insert('orders', $data);
            setFlash('success', 'オーダーを発行しました');
        }
...
<div class="fade-in">
    <div class="mb-3">
        <a href="?page=orders" class="text-decoration-none"><i class="bi bi-arrow-left me-1"></i>オーダー管理に戻る</a>
        <h4 class="fw-bold mt-2 mb-0"><?= $id ? 'オーダー編集' : '新規オーダー' ?></h4>
    </div>
    <form method="POST" class="card">
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    <label class="form-label required">オーダー種別</label>
                ...
                <div class="col-md-6">
                    <label class="form-label">マスタから選択</label>
                    <select class="form-select" id="masterSelect" onchange="applyMaster()">
                        <option value="">-- 直接入力またはマスタから選択 --</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label required">オーダー名</label>
                    <input type="text" name="order_name" class="form-control" value="<?= h($o['order_name'] ?? '') ?>" required id="orderName">
                </div>
                <div class="col-md-3">
                    <label class="form-label">カテゴリ</label>
                    <input type="text" name="order_category" class="form-control" value="<?= h($o['order_category'] ?? '') ?>" id="orderCat">
                </div>
                <div class="col-md-9">
                    <label class="form-label">詳細</label>
                    <input type="text" name="order_detail" class="form-control" value="<?= h($o['order_detail'] ?? '') ?>">
                </div>
                <div class="col-md-2">
                    <label class="form-label">数量</label>
                    <input type="number" name="quantity" class="form-control" value="<?= h($o['quantity'] ?? 1) ?>" step="0.01" id="quantity" oninput="updateOrderTotal()">
                </div>
                <div class="col-md-2">
                    <label class="form-label">単位</label>
                    <input type="text" name="unit" class="form-control" value="<?= h($o['unit'] ?? '') ?>" id="orderUnit">
                </div>
                <div class="col-md-3">
                    <label class="form-label">単価</label>
                    <input type="number" name="unit_price" class="form-control" value="<?= h($o['unit_price'] ?? 0) ?>" id="unit_price" oninput="updateOrderTotal()">
                </div>
                <div class="col-md-3">
                    <label class="form-label">合計金額</label>
                    <input type="number" name="total_price" class="form-control" value="<?= h($o['total_price'] ?? 0) ?>" id="total_price">
                </div>
                <div class="col-md-3">
                    <label class="form-label">用法・頻度</label>
                    <input type="text" name="frequency" class="form-control" value="<?= h($o['frequency'] ?? '') ?>" placeholder="BID, TID等">
                </div>
                <div class="col-md-3">
                    <label class="form-label">期間</label>
                    <input type="text" name="duration" class="form-control" value="<?= h($o['duration'] ?? '') ?>" placeholder="7日間等">
                </div>
                <div class="col-md-3">
                    <label class="form-label">投与経路</label>
                ...
                <div class="col-12">
                    ...
                    <textarea name="notes" class="form-control" rows="2"><?= h($o['notes'] ?? '') ?></textarea>
                </div>
            </div>
        </div>
        <div class="card-footer d-flex justify-content-between">
            <a href="?page=orders" class="btn btn-secondary"><i class="bi bi-arrow-left me-1"></i>キャンセル</a>
            <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i><?= $id ? 'オーダー更新' : 'オーダー発行' ?></button>
        </div>
    </form>
</div>
<script>
